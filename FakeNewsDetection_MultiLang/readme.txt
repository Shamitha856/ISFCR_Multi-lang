...
3).py file having implementation code of the web interface is under the "Web Interface" folder inside the main project folder.
	Inturn Web Interface has 2 other folders static (containing style file for UI) and templates (containing an html file for web interface), retain this directory structure as it is(as they are uploaded onto the repo) to avoid issues related to file paths. app.py is the API file for the frontend.(Present under the Web Interface folder)
...
6)Files required to run the web interface: pickle files and 'Web Interface' folder mentioned in point 3 + requirements. Once all 3 are ready inplace run app.py(inside Web Interface folder) either using Spyder(recommended) or as running any normal .py script files. This step completes rendering web interface on port 5000(mentioned in .py files).
