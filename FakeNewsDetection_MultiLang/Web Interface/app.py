...
vectorization_hindi = TfidfVectorizer()
loaded_hindi_model = pickle.load(open('model_hindi_pac.pkl', 'rb'))
dataframe_hindi = pd.read_csv('final_df_hindi.csv')
x_hindi = dataframe_hindi['long_description']
y_hindi = dataframe_hindi['real/fake']
x_train_h, x_test_h, y_train_h, y_test_h = train_test_split(x_hindi, y_hindi, test_size=0.2, random_state=0)

def fake_news_det_hindi(news):
    xv_train = vectorization_hindi.fit_transform(x_train_h)
    xv_test = vectorization_hindi.transform(x_test_h)
    input_data = [news]
    vectorized_input_data = vectorization_hindi.transform(input_data)
    prediction = loaded_hindi_model.predict(vectorized_input_data)
    return prediction

vectorization_kannada = TfidfVectorizer()
loaded_model_kannada = pickle.load(open('model_kannada_pac.pkl', 'rb'))
dataframe_kannada = pd.read_csv('final_df_kannada.csv')
x_kannada = dataframe_kannada['text_name']
y_kannada = dataframe_kannada['real/fake']
x_train_k, x_test_k, y_train_k, y_test_k = train_test_split(x_kannada, y_kannada, test_size=0.2, random_state=0)

def fake_news_det_kannada(news):
    xv_train = vectorization_kannada.fit_transform(x_train_k.values.astype('U'))
    xv_test = vectorization_kannada.transform(x_test_k.values.astype('U'))
    input_data = [news]
    vectorized_input_data = vectorization_kannada.transform(input_data)
    prediction = loaded_model_kannada.predict(vectorized_input_data)
    return prediction
...
@app.route('/predict_hindi_text', methods=['POST'])
# A function to predict the text based news implemented using passive aggressive classifier
def predict_hindi_text():
    if request.method == 'POST':
        message = request.form['news']
        ...
        if message.startswith("https://" or "http://" or "www."):
            print('URL')
            pred2=['Url']
            return render_template('index.html', prediction4=pred2)
        elif language == 'en' or language == 'kn':
            print("En or Kn")
            pred2 = ['enORkn_text']
            return render_template('index.html', prediction4=pred2) 
        else:
            pred2 = fake_news_det_hindi(message)
            print(pred2)
            return render_template('index.html', prediction4=pred2)        
    else:
        return render_template('index.html', prediction4=pred2)
    
@app.route('/predict_kannada_text', methods=['POST'])
# A function to predict the text based news implemented using passive aggressive classifier
def predict_kannada_text():
    if request.method == 'POST':
        message = request.form['news']
        language = langid.classify(message)
        #language = detect_langs(message)
        ...
        if message.startswith("https://" or "http://" or "www."):
            print('URL')
            pred2=['Url']
            return render_template('index.html', prediction5=pred2)
        elif language == 'en' or language == 'hi':
            print("En or Hn")
            pred2 = ['enORhn_text']
            return render_template('index.html', prediction5=pred2) 
        else:
            pred2 = fake_news_det_kannada(message)
            print(pred2)
            return render_template('index.html', prediction5=pred2)        
    else:
        return render_template('index.html', prediction5=pred2)
